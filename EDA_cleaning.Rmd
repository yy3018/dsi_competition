...
final1 = rbind(final1,three_days)

###############
data1=left_join(x=final1,y=stay_home_time,by="state",all=T) %>% na.omit()
days = as.Date(as.character(data1$date), format="%m-%d-%Y")-as.Date(data1$time,format = "%Y-%d-%b" )
days = as.numeric(days)
final = cbind(data1,days) %>% 
  filter(days<=14 & days>=0) %>% 
