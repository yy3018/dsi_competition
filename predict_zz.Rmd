...
fig1 = vis %>% 
  group_by(group) %>% 
  ggplot(aes(x = time, y = I,color = group, fill = group)) + 
  geom_line()+
  geom_hline(yintercept=0.1)+
  annotate("text", x=20, y=0.12, label="Healthcare system capacity") 
ggplotly(fig1)
```
...
fig2 = vis_seir %>% 
  ...
  scale_color_discrete(breaks = c('S','E','I','R'))+
      ...
      theme(legend.text = element_text(angle = 45, size = 10, hjust = 3, vjust = 3))
